...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>AnBaToCom Hub V1 chào ae<Color=/>"):Display()
    wait(.5)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Đăng ký kênh Nhật Roblox Mods để xem video mới nhất nhé<Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "AnBaToCom Hub V1", HidePremium = false, IntroText = "AnBaToCom Hub V1⚜️", SaveConfig = true, ConfigFolder = "NightHub"})



OrionLib:MakeNotification({
    Name = "AnBaToCom Hub V1",
...
W:AddLabel("AnBaToCom Hub V1 SCRIPT")
W:AddLabel("DiscordAnBaToCom Hub V1")
local Section = W:AddSection({
    Name = "Status"
})
...
C:AddButton({
    ...
    Callback = function(value)
        setclipboard("https://discord.com/invite/2eXDsDju
    end
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "AnBaToCom Hub V1(Noitification)",
